...
- **`GhostTreeBstackAllocator::dealloc` — double-free undetected** (`alloc` feature): `dealloc` now reads the first 8 bytes of the block before zeroing; if they equal `align_up_len(slice.len())` the block is already in the free tree (its AVL size field is stored there) and `InvalidInput` is returned.

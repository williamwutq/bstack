...
- **`FirstFitBStackAllocator::alloc` — release allocator lock before tail push** (`alloc` + `set` + `atomic` features): In the no-free-block path, the allocator mutex is now released before calling `BStack::push` / `bstack_push`. The push is a single atomic `BStack` call serialised by `BStack`'s own write lock; holding the allocator mutex across it only increases contention without adding correctness. On the error path the mutex was already released before returning; the success path now matches. No behavioural change.

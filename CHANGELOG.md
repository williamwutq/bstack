...
- **`BStackOwnedSlice<'a, A: BStackAllocator>`** (`alloc` feature): New ownership handle for one allocator-managed region. Non-`Copy`, non-`Clone` — an allocation has exactly one owner. `realloc` and `dealloc` consume it by value, so use-after-free and use-after-realloc are compile errors. No direct I/O; obtain a borrowed I/O view via `as_slice(&self)` (shared) or `as_slice_mut(&mut self)` (exclusive). Constructors: `from_raw_parts`, `from_raw_range`, `empty`. Accessors: `start`, `end`, `len`, `is_empty`, `range`, `allocator`. Drop is a no-op; the allocation persists until explicitly freed.
- **`BStackRange`** (`alloc` feature): Raw `(offset, len)` coordinate pair. `Copy`, `Clone`, `PartialOrd`, `Ord`, `Hash`. The serialization/persistence representation — store it on disk, pass it across sessions, or hold it in data structures that should not perform I/O. To do anything with the region it describes, cast it `unsafe`ly to `BStackSlice` (for I/O) or `BStackOwnedSlice` (for allocation operations). Constructors: `new` (checked, caps on overflow), `unsafe from_raw_parts`. Serialization: `to_bytes` / `from_bytes` (16-byte little-endian layout). `From<Range<u64>>` / `Into<Range<u64>>`.
...
- **`BStackAllocator::Allocated<'a>` constraint changed** (`alloc` feature) *(breaking)*: Was `Copy + TryInto<BStackSlice<'a, Self>>`. Now `Into<BStackOwnedSlice<'a, Self>> + Send`. The `Copy` requirement is gone; ownership semantics are enforced instead. **Migration:** Set `type Allocated<'a> = BStackOwnedSlice<'a, Self>` in all `impl BStackAllocator` blocks.

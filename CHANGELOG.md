...
- **C bindings: in-place mutations are now crash-atomic via the write-in-progress journal (parity with Rust).** The C library previously used the pre-0.4.0 16-byte header (magic `BSTK\x00\x01\x0f\x00`) and overwrote payload bytes in place with no journal, so a crash mid-write could leave a torn region. It now implements the full 0.4.0 format — 32-byte header (`clen | wip_ptr | wip_aux`), the derived-atomicity single-block fast path, and the `Set` / `Repeat` / `Copy` / `SpliceGrow` / `SpliceShrink` journal modes with recovery on `bstack_open` — so `bstack_set`, `bstack_zero`, `bstack_swap`, `bstack_cas`, `bstack_copy`, `bstack_cross_exchange`, `bstack_process`, the `crds` family, and the tail-replace family (`bstack_atrunc` / `bstack_splice` / `bstack_replace`) are each crash-atomic on the same terms as their Rust counterparts. See `algos/WIP.md`.

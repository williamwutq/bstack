...
static int test_slice_cas_on_self_guard_swaps(void)
{
    char tmp[64]; make_tmp(tmp, sizeof tmp);
    bstack_t *bs = bstack_open(tmp); CHECK(bs);
    first_fit_bstack_allocator_t *a = first_fit_bstack_allocator_new(bs);
    ...
    bstack_slice_t target;
    CHECK(bstack_allocator_alloc((bstack_allocator_t *)a, 2, &target) == 0);
    uint8_t target_buf[2] = {1, 2};
    CHECK(bstack_slice_write(target, target_buf, 2) == 0);
    ...
    uint8_t old_buf[2];
    int ok = 0;
    CHECK(bstack_slice_cas_on(target, target, expected, 2, new_bytes, 2,
                              old_buf, &ok) == 0);
    CHECK(ok == 1);
    CHECK(memcmp(old_buf, target_buf, 2) == 0);
    uint8_t rbuf[2];
...
int main(void)
{
    ...
    /* Hardening: corrupt free-list links are rejected, not chased. The shared
     * free-list paths validate, so run in either build. */
    ...
    T(test_slice_cas_on_self_guard_swaps);

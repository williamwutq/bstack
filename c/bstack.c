...
static const uint8_t  MAGIC[8]        = {'B','S','T','K', 0, 1, 17, 0};
...
int bstack_resize(bstack_t *bs, uint64_t target, uint64_t *out_initial_len)
{
    ...
    if (write_committed_len(bs->fd, &bs->clen, target) != 0 ||
        plat_durable_sync(bs->fd) != 0)
    {
        /* Best-effort rollback. The cache is reset up front so it reflects the
         * rolled-back file even if the header rewrite below fails. */
        plat_ftruncate(bs->fd, raw_size);
        bs->clen = data_size;
 ...
 *
 * Shared by bstack_extend_sparse[_batched] and their try_ variants.
 * ---------------------------------------------------------------------- */

/* qsort comparator: order iovec descriptors by ascending offset. */
                       ...
                       void *ctx, uint64_t *out_initial_len)
{
    ...
    /* The grown bytes sit beyond the committed length until the header write
     * below, so a crash rolls back by truncation. */
...
static inline int crds_do_swap(bstack_fd_t fd,

...
/* Shared argument validation for the cas_on family: same bstack_t, expected
 * sized to guard, new_bytes sized to s.  Sets errno on rejection. */
static int slice_cas_check(bstack_slice_t s, bstack_slice_t guard,
                           size_t expected_len, size_t new_bytes_len)
{
...
{
    if (slice_cas_check(s, guard, expected_len, new_bytes_len) != 0)
        return -1;
...
static int alff_recovery(bstack_t *bs)
{
    ...
    while (pos < stack_len) {
         ...
         *     back by truncating to pos — restoring the pre-grow tail the failed
         *     realloc already handed back to the caller.
         *   * Anything else -> genuine mid-arena corruption; fail loudly rather
         *     than silently discard the data that follows. */
                          ...
                          uint64_t new_len, bstack_slice_t *out)
{
    ...
    if (slice.offset + aligned_current_len == stack_len - ALFF_BLOCK_FTR_SIZE) {
        ...
        if (aligned_new_len > aligned_current_len) {
             ...
             * block-walking recovery cannot parse the torn intermediate (a fault
             * between the header rewrite and the discard leaves header, footer,
             * and physical size disagreeing). Narrowing only the user-visible
             * length (an oversized block, as a non-tail shrink does) needs no
             * writes; the tail is reclaimed when the block is freed. */
        }
        out->allocator = self;
        out->offset    = slice.offset;
        out->len       = new_len;
        ...
        /* Case 4: try to merge with the free right neighbour in place */
        {
            ...
            if (next_block <= stack_len - ALFF_BLOCK_FTR_SIZE - ALFF_MIN_PAYLOAD) {
                ...
                if ((next_hdr[8] & 1) != 0
                    ...
                    if (alff_set_recovery_needed(a->bs) != 0) { MUTEX_UNLOCK(a); return -1; }
                    if (alff_unlink_from_free_list(a->bs, next_block, stack_len) != 0) { MUTEX_UNLOCK(a); return -1; }
 ...
 * free list is the only way to tell that a block is free. A list holds at most
 * total_blocks nodes, so that count alone bounds the walk and serves as the
 * cycle check. */
                          ...
                          size_t *out_len, void *ctxp)
{
    ...
    if (c->pending) {
        ...
        if (val == SLAB_SENTINEL
            ...
            /* End of list, or a malformed or cyclic free list: stop, reporting

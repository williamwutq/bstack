...
/* Slice-relative byte offset of element at index (0-based): the coordinate
 * accepted by the bstack_slice_* read/write/zero calls, which are relative to
 * the start of this vec's block. */
...
/*
 * Absolute payload offset of logical byte index within the backing bstack_t —
 * the coordinate accepted by bstack_copy, bstack_cross_exchange, and
 * bstack_repeat.  Equal to the block's start plus the 16-byte header plus
 * index; distinct from the slice-relative bytevec_elem_offset, and must be

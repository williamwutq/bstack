//! Single-threaded throughput benchmark for the `bstack` disk allocators,
//! built *without* the `atomic` feature.
//!
//! This is the non-atomic twin of [`alloc.rs`](alloc.rs): same temp-file
//! plumbing, `make_allocator!` constructor macro, weighted operation
//! generator, and size generator. It differs only in that it never spawns
//! threads — without `atomic` the allocators are not `Sync`, so there is
//! nothing to compare across thread counts. Every sample is a single-threaded
//! replay of the pre-generated decision stream, giving the un-serialized
//! (no `Mutex`) baseline that `alloc.rs`'s `1t` figures should track.
//!
//! # Configuration
//!
...
#[cfg(feature = "atomic")]
compile_error!(
    "benches/alloc_non_atomic.rs requires the `atomic` feature to be disabled; use the `alloc` bench for the atomic/multi-threaded comparison"
);
#[cfg(feature = "debug-no-sync")]
compile_error!(
    "benches/alloc_non_atomic.rs requires the `debug-no-sync` feature to be disabled; benchmarking without the durable fsync is pointless"
);
...
impl BenchConfig {
    fn from_env() -> Self {
        ...
        eprintln!(
            "bstack alloc bench (non-atomic): op={} size={} max={} pre_alloc={} seed={}",
            cfg.op_label, cfg.size_label, cfg.max, cfg.pre_alloc, cfg.seed,
        );
        cfg
    }
}
...
/// Benchmark one allocator, single-threaded.
///
/// Decisions (ops + sizes) are drawn from a seeded RNG outside the timed
/// region; the live set is pre-populated (untimed), and only the operation
/// replay is timed.
    ...
    SG: Fn(&mut StdRng) -> u64,
{
    ...
    group.bench_function(bench_name, |b| {
        b.iter_custom(|iters| {
            ...
            let pre_sizes: Vec<u64> = (0..cfg.pre_alloc).map(|_| size_gen(&mut rng)).collect();
            let sizes: Vec<u64> = (0..iters as usize).map(|_| size_gen(&mut rng)).collect();

            let mut live: Vec<BStackOwnedSlice<'_, A>> = Vec::with_capacity(pre_sizes.len());
            let mut idx_rng = StdRng::seed_from_u64(cfg.seed);
            ...
            for (&len, &op) in sizes.iter().zip(all_ops.iter()) {
                match op {
                    Operation::Alloc => {
                        if let Ok(sl) = alloc.alloc(len) {
                            live.push(sl);
                    ...
                    Operation::Realloc => {
                        if live.is_empty() {
                            ...
                            let i = idx_rng.random_range(0..live.len());
...
/// Register `make` under `group`, drawing sizes from the active [`SizeDist`]
/// (up to `cfg.max`) and ops from the active [`OpMix`]. The benchmark id
/// encodes the workload so distinct configs don't clobber each other's
/// baselines, and keeps the `1t` suffix used by `alloc.rs` so the two
/// benches' single-threaded figures line up directly.
macro_rules! register {
    ($c:expr, $group:expr, $make:expr) => {{
        ...
        let name = format!("{}/{}/1t", cfg.op_label, cfg.size_label);
        bench_allocator(
            $c,
            $group,
            &name,
...
fn bench_slab_16(c: &mut Criterion) {
    register!(
        c,
        "alloc_non_atomic/slab_16",
        make_allocator!(SlabBStackAllocator, 16)
    );
}
...
fn bench_slab_24(c: &mut Criterion) {
    register!(
        c,
        "alloc_non_atomic/slab_24",
        make_allocator!(SlabBStackAllocator, 24)
    );
}
...
fn bench_slab_32(c: &mut Criterion) {
    register!(
        c,
        "alloc_non_atomic/slab_32",
        make_allocator!(SlabBStackAllocator, 32)
    );
}
...
fn bench_slab_64(c: &mut Criterion) {
    register!(
        c,
        "alloc_non_atomic/slab_64",
        make_allocator!(SlabBStackAllocator, 64)
    );
}
...
fn bench_checked_slab_16(c: &mut Criterion) {
    register!(
        c,
        "alloc_non_atomic/checked_slab_16",
...
fn bench_checked_slab_24(c: &mut Criterion) {
    register!(
        c,
        "alloc_non_atomic/checked_slab_24",
...
fn bench_checked_slab_32(c: &mut Criterion) {
    register!(
        c,
        "alloc_non_atomic/checked_slab_32",
...
fn bench_checked_slab_64(c: &mut Criterion) {
    register!(
        c,
        "alloc_non_atomic/checked_slab_64",
...
fn bench_checked_slab_128(c: &mut Criterion) {
    register!(
        c,
        "alloc_non_atomic/checked_slab_128",

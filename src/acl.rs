...
/// Dispatch one allocator metadata op through its held authority.
///
/// `alloc_meta!(self, op, op_as, args...)` expands to
/// `self.stack.op_as(&self.alloc_auth, args...)` with the
/// `expensive-slice-access-control` feature — presenting the allocator's *held*
/// real [`BStackAllocAuthority`] to the token op, so its own permanently
/// `Alloc`-marked metadata is admitted — or to the plain `self.stack.op(args...)`
/// without it. There are no `meta_*` methods; this expands inline at the call
/// site, so an op whose call site is `atomic`-gated needs no gating here.
///

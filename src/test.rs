...
mod first_fit_tests {
    use crate::BStack;
    use crate::alloc::{BStackAllocStats, BStackAllocator, FirstFitBStackAllocator};
    use std::sync::atomic::{AtomicU64, Ordering};
    ...
    fn stats_counts_free_and_in_use_through_coalescing() {
        ...
        // c's left neighbour (the merged a+b block) is free, and the merge now
        // reaches the tail, so dealloc discards the whole arena, leaving no
        // free node behind.
        alloc.dealloc(c).unwrap();
        assert_eq!(
            alloc.stats().unwrap(),

...
impl SlabBStackAllocator {
    ...
    ///
    /// A plain slab block carries no per-block state; chasing the
    /// singly-linked free list from `free_head` is the only way to tell that a
    /// block is free. This walks that list in one
    /// [`BStack::get_batched_gen`] sequence, validating each node before
    /// following it: in bounds, and `block_size`-aligned. `in_use_blocks` is
    /// then `total_blocks - free_blocks`, for `total_blocks` the arena size
    /// divided by `block_size`.
    ///
    /// A list holds at most `total_blocks` nodes. That count alone bounds the
    /// walk and serves as the cycle check: a malformed pointer ends it where
    /// it is found, and a cycle ends it at `total_blocks`, reporting the whole
    /// arena free.
    ///
    /// Reads happen under one held shared lock. The counts are therefore a
    /// consistent snapshot even under concurrent `alloc`/`dealloc`, and this
    /// allocator carries no allocator-level lock at all.
    ///
    /// # Errors
    ///
    /// Any [`io::Error`] from the underlying [`BStack::get_batched_gen`] call.
    pub fn stats(&self) -> io::Result<BStackAllocStats> {
        ...
        self.stack.get_batched_gen(|| {
            if pending {
                ...
                {
                    // End of list, or a malformed or cyclic free list: stop,
            ...
            // and nothing else.
            //
            // SAFETY: `buf` is declared before this call and never moved, so it
            // outlives the whole sequence; each buffer is consumed before the
            // closure runs again, so no other access overlaps it.
            let scratch: &mut [u8] = &mut buf[..];
            Some((next_read, unsafe { &mut *(scratch as *mut [u8]) }))
        })?;
...
mod tests {
    ...
    fn stats_empty_arena_is_all_zero() {
        ...
        let _g = Guard(path);
        let alloc = SlabBStackAllocator::new(stack, 16).unwrap();
        assert_eq!(alloc.stats().unwrap(), BStackAllocStats::default());
    }
    ...
    fn stats_counts_free_and_in_use_blocks() {
        ...
        let c = alloc.alloc(8).unwrap();
        alloc.dealloc(b).unwrap();
        // a, c in use (16 B each); b free (16 B). A single-block dealloc always
        // goes to the free list, tail or not.
        assert_eq!(
            alloc.stats().unwrap(),
            BStackAllocStats {
                free_blocks: 1,
    ...
    fn stats_multi_block_allocation_counts_each_block_in_use() {
        ...
        // served by a tail extend, so with no per-block state to say
        // otherwise, both blocks read as in-use.
        let alloc = SlabBStackAllocator::new(stack, 16).unwrap();

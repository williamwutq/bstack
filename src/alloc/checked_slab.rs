...
/// | `alloc_bulk` *(needs `atomic`)* | Partial | Yes (`process_gen`+`extend`+`set_batched`) | detached/extended blocks leaked, `recover` reclaims |
/// | `dealloc_bulk` *(needs `atomic`)* | Partial | Yes (`set_batched`+`cross_exchange`) | freed batch leaked, `recover` reclaims |
///
...
impl CheckedSlabBStackAllocator {
    ...
    pub fn new(stack: BStack, data_size: u64) -> io::Result<Self> {
        ...
        if data_size < Self::MIN_DATA_SIZE {
            return Err(io_error!(
                InvalidInput,
                "data_size ({data_size}) must be >= {}",
                Self::MIN_DATA_SIZE
            ));
        }
        let block_size = data_size
            .checked_add(Self::OVERHEAD)
            .ok_or_else(|| io_error!(InvalidInput, "data_size is too large (overflows u64)"))?;
    ...
    ///
    /// Advances `free_head` then claims the block in one call: the overhead is
    /// set to `IN_USE_BIT | num_blocks`, and with [`true`] the data bytes
    /// are zeroed in that same write ([`false`] writes the overhead alone
    /// and leaves the data as the previous occupant left it). A crash between the
    ...
    /// The final claim write happens afterwards on the now-detached block,
    /// which is exclusively owned by this call.
    #[cfg(feature = "atomic")]
    fn pop_and_claim_block(&self, num_blocks: u64, init: bool) -> io::Result<Option<u64>> {
        ...
        // holding whatever the previous occupant left (including the stale
        // free-list `next` pointer at `data[0..8]`).
        let overhead = (Self::IN_USE_BIT | num_blocks).to_le_bytes();
        if init {
            let mut block_buf = vec![0u8; self.block_size as usize]; // safe: validated in new/open
            block_buf[..8].copy_from_slice(&overhead);
            self.stack.set(head, block_buf)?;
        } else {
            self.stack.set(head, overhead)?;
        }
        Ok(Some(head))
    }
    ...
    fn pop_and_claim_block(&self, num_blocks: u64, init: bool) -> io::Result<Option<u64>> {
        ...
        alloc_meta!(self, set, set_as, Self::FREE_HEAD_OFFSET, &prefix[8..16])?;
        // Mark in-use and, when the caller is owed zeroes, scrub the data in the
        // same write. `false` writes just the overhead word — still one
        // `set`, but `block_size - OVERHEAD` fewer bytes — and leaves the data
        // holding whatever the previous occupant left (including the stale
    ...
    fn write_free_run(&self, first_block: u64, count: u64) -> io::Result<()> {
        ...
        let buf_size = usize::try_from(total)
...
impl CheckedSlabBStackAllocator {
    ...
    ///
    /// The two differ only in the claim write of the free-list-reuse path:
    /// [`true`] fuses the overhead word and a full block scrub into one
    /// `set`, [`false`] writes the overhead word alone. Both are a single
    /// [`BStack`] call, so the crash-consistency table is identical. The
    /// tail-extend paths do not differ at all — [`BStack::extend`] zero-fills by
    /// `set_len` on a sparse file, at no cost.
    fn alloc_impl(&self, len: u64, init: bool) -> io::Result<BStackOwnedSlice<'_, Self>> {
        ...
        if num_blocks == 1
            && let Some(block_start) = self.pop_and_claim_block(1, init)?
        {
            // SAFETY:
    ...
    ///
    /// [`false`] drops the [`BStack::zero`] of bytes newly exposed inside
    /// the block run the caller already owns — one whole durable sync per grow —
    /// and propagates into the fresh allocation of the non-tail grow. Everything
    /// that maintains block metadata (the overhead commit point, the tail
    /// extend/discard, the free-list splices) is unconditional, so the
    /// crash-consistency table applies unchanged.
    fn realloc_impl<'a>(
        &'a self,
        ...
        let result = (|| -> io::Result<BStackOwnedSlice<'a, Self>> {
            ...
            if new_n > old_n {
                ...
                let new_slice = self.alloc_impl(new_len, init)?;
            ...
            let excess_start = block_start
                .checked_add(new_backing)
                .ok_or_else(|| io_error!(InvalidInput, "excess start overflows u64"))?;
...
///
/// Mirrors [`SlabBStackAllocator`](super::SlabBStackAllocator)'s bulk strategy —
/// per-request handles, free list first then one tail extend — and additionally
/// maintains every block's 8-byte overhead tag: recycled and extended blocks are
/// marked in-use as part of the same batch, and freed blocks have their overhead
/// cleared to zero *before* the free-list splice, so a crash leaves either the
/// intact original or zero-overhead leaked blocks that
/// [`recover`](CheckedSlabBStackAllocator::recover) reclaims.
///
...
impl BStackBulkAllocator for CheckedSlabBStackAllocator {
    ...
    fn alloc_bulk(
        ...
        for (&len, &n) in lengths.iter().zip(counts.iter()) {
            ...
            // the data region begins at `block_start + OVERHEAD` and spans
            // `len ≤ n * block_size − OVERHEAD` bytes.
            result.push(unsafe {
                BStackOwnedSlice::from_raw_parts(self, block_start + Self::OVERHEAD, len)
            });
        }
    ...
    ///
    /// Each handle's overhead is read and validated first (a cleared in-use bit
    /// is a double-free, a repeated block within the batch is a double-free too),
    /// so a bad batch is rejected whole with every handle returned before any
    /// write. The freed blocks are then threaded into one chain — each block's
    /// overhead cleared and `next` pointer set together — with **one**
    /// [`BStack::set_batched`], and the run is spliced onto the free list with a
    /// single atomic [`BStack::cross_exchange`]. Because every scrub is staged
    /// before the splice, a crash leaves the zero-overhead blocks as leaks that
    /// [`recover`](Self::recover) reclaims; the free list never sees a torn chain.
    /// Null sentinel handles are ignored.
    ///
    /// Unlike the single-item [`dealloc`](BStackAllocator::dealloc), a run at the
    /// tail is **not** discarded — every block goes to the free list.
    ///
    /// # Errors
    ///
    /// * [`io::ErrorKind::InvalidInput`] — a handle sits below the overhead
    ///   prefix, or a block is already free / repeated (double-free); the whole
    ///   batch is rejected and every handle returned.
    /// * [`io::ErrorKind::InvalidData`] — an in-use overhead records a zero block
    ///   count (metadata corrupt).
    ///
    /// # Atomicity
    ///
    /// The staging batch and the splice are each crash-atomic. A crash or I/O
    /// failure after staging begins leaks the freed blocks as zero-overhead
    /// blocks (reclaimed by `recover`); no handle can then be returned, so
    ...
    fn dealloc_bulk<'a>(
        ...
        let result = (|| -> io::Result<()> {
            // Validate and expand every allocation into its block offsets before
            // touching anything, so a bad batch returns all handles intact.
            let mut blocks: Vec<u64> = Vec::new();
            let mut seen: HashSet<u64> = HashSet::new();
            for s in &slices {
                if s.is_empty() && s.start() == 0 {
                    continue;
                }
                let block_start = s.start().checked_sub(Self::OVERHEAD).ok_or_else(|| {
                    io_error!(
                        InvalidInput,
                        "dealloc_bulk: slice start is below the overhead prefix"
                    )
                })?;
                let overhead = self.read_overhead(block_start)?;
                if overhead & Self::IN_USE_BIT == 0 {
                    return Err(io_error!(
                        InvalidInput,
                        format!(
                            "dealloc_bulk: double free detected: block at {block_start} is not marked in use"
                        )
                    ));
                }
                let n = overhead & Self::BLOCKS_MASK;
                if n == 0 {
                    return Err(io_error!(
                        InvalidData,
                        "dealloc_bulk: in-use block records a zero block count; metadata corrupt"
                    ));
                }
                ...
                block_start.checked_add(backing).ok_or_else(|| {
                    io_error!(
                        InvalidInput,
...
/// Claiming a free-list block without rewriting its data.
///
/// [`alloc`](BStackAllocator::alloc) claims a recycled block with one `set` of
/// a whole `block_size` staging buffer, fusing the `IN_USE_BIT | num_blocks`
/// overhead word with a scrub of every data byte.
/// [`alloc_uninit`](BStackUninitAllocator::alloc_uninit) writes the overhead
/// word alone: the same single [`BStack`] call, but `block_size - 8` fewer bytes
/// to write and journal, and no heap staging buffer.  The saving therefore
/// scales with the block size rather than removing a durable sync.
///
/// Note that this allocator scrubs on *free*, not on claim — `dealloc` (and
/// `recover`, when it reclaims a leaked block) lays a fully-zeroed image over
/// the freed run — so in practice a block handed out by `alloc_uninit` still
/// reads back as zero.  Callers must not rely on that: it is an implementation
/// detail of the current free path, and the trait promises only unspecified
/// contents.
///
/// [`realloc_uninit`](BStackUninitAllocator::realloc_uninit) additionally drops
/// the [`BStack::zero`] of bytes newly exposed inside the caller's own block
/// run, which *is* a whole durable sync per grow.
///
/// Crash consistency is unchanged in both cases: every path issues the same
/// [`BStack`] calls in the same order, so the tables on
/// [`alloc`](BStackAllocator::alloc) and [`realloc`](BStackAllocator::realloc)
/// apply verbatim.  Recovery is unaffected too — its arena scan reads only the
/// overhead words at block starts and strides over live blocks by their
/// recorded block count, so it never inspects a live block's data.
#[cfg(feature = "set")]
...
mod tests {
    ...
    fn alloc_uninit_of_a_fresh_block_still_reads_zero() {
        ...
        // reads back as zero even though nothing scrubbed it.
        let (stack, path) = empty_stack();
        let _g = Guard(path);
        let alloc = CheckedSlabBStackAllocator::new(stack, 32).unwrap();
    ...
    #[test]
    fn alloc_uninit_reuses_a_freed_block() {
        // `dealloc` writes a fully-zeroed image over the freed run, so a recycled
        // block happens to read back as zero here even though `alloc_uninit`
        // scrubs nothing. That is a property of this allocator, not a promise:
        ...
        let start = a.start();
        // A later allocation keeps `a` off the tail, so freeing it goes through
        // the free list rather than a plain discard.
        let _pin = alloc.alloc(32).unwrap();
        alloc.dealloc(a).unwrap();

        let mut b = alloc.alloc_uninit(32).unwrap();
        assert_eq!(b.start(), start, "the freed block must be the one reused");
        assert_eq!(b.len(), 32);
        b.write([0xC3u8; 32]).unwrap();
        assert_eq!(b.read().unwrap(), vec![0xC3u8; 32]);
    }
    ...
    #[test]
    fn alloc_uninit_keeps_the_block_recoverable() {
        // The overhead word is still written, so the freed/live split survives a
        // reopen and `recover` accounts for every block.
        let (stack, path) = empty_stack();
    ...
    fn realloc_uninit_preserves_existing_bytes_on_grow() {
        ...
        // A second allocation keeps the first off the tail: forces the move path.
    ...
    fn recover_reclaims_leaked_free_block() {
        ...
        crate::acl::alloc_meta!(alloc, set, set_as, 40, 0u64.to_le_bytes()).unwrap();
    ...
    fn recover_is_idempotent() {
        ...
        alloc.dealloc(b).unwrap();
        crate::acl::alloc_meta!(alloc, set, set_as, 40, 0u64.to_le_bytes()).unwrap(); // leak block 64
        assert_eq!(alloc.recover().unwrap(), 0);
    ...
    fn dealloc_and_realloc_reject_a_handle_from_another_instance() {
        ...
        let _g2 = Guard(p2);
        let a1 = CheckedSlabBStackAllocator::new(s1, 64).unwrap();
        let a2 = CheckedSlabBStackAllocator::new(s2, 64).unwrap();
...
mod bulk_tests {
    ...
    fn temp_path() -> std::path::PathBuf {
        ...
        let id = COUNTER.fetch_add(1, Ordering::Relaxed);
        let pid = std::process::id();
        std::env::temp_dir().join(format!("bstack_checked_slab_bulk_{pid}_{id}.bin"))
    }
    ...
    fn alloc_bulk_empty_returns_empty() {
        ...
        let _g = Guard(path);
        let alloc = CheckedSlabBStackAllocator::new(stack, 24).unwrap();
        assert!(alloc.alloc_bulk([]).unwrap().is_empty());
    }
    ...
    #[test]
    fn alloc_bulk_zero_and_null_entries() {
        let (stack, path) = empty_stack();
        let _g = Guard(path);
        let alloc = CheckedSlabBStackAllocator::new(stack, 24).unwrap();
        let slices = alloc.alloc_bulk([0u64, 24, 0]).unwrap();
    ...
    fn alloc_bulk_oversized_entries_are_contiguous_and_independently_freeable() {
        let (stack, path) = empty_stack();
        let _g = Guard(path);
        // data_size = 24, block_size = 32; 60 bytes needs 3 blocks
        // (60 + 8 overhead = 68, div_ceil 32 = 3).
        let alloc = CheckedSlabBStackAllocator::new(stack, 24).unwrap();
        let mut slices = alloc.alloc_bulk([60u64, 24]).unwrap();
        assert_eq!(slices[0].len(), 60);
        slices[0].write([0xAAu8; 60]).unwrap();
        assert_eq!(slices[0].read().unwrap(), vec![0xAAu8; 60]);
        // Each frees on its own; the multi-block overhead is read back correctly.
    ...
    fn alloc_bulk_recycled_blocks_read_back_zero() {
        ...
        let _g = Guard(path);
        let alloc = CheckedSlabBStackAllocator::new(stack, 24).unwrap();
        let mut a = alloc.alloc_bulk([24u64, 24, 24]).unwrap();
        for s in &mut a {
            s.write([0x5Au8; 24]).unwrap();
        }
    ...
    fn alloc_bulk_prefers_free_list_then_extends() {
        ...
        let _g = Guard(path);
        let alloc = CheckedSlabBStackAllocator::new(stack, 24).unwrap();
        let a = alloc.alloc_bulk([24u64, 24]).unwrap();
        let mut freed: Vec<u64> = a.iter().map(|s| s.start()).collect();
        freed.sort_unstable();
        alloc.dealloc_bulk(a).unwrap();

        let b = alloc.alloc_bulk([24u64, 24, 24, 24]).unwrap();
    ...
    fn dealloc_bulk_frees_and_recover_reports_clean() {
        ...
        let _g = Guard(path.clone());
        let alloc = CheckedSlabBStackAllocator::new(stack, 24).unwrap();
        // Mixed single- and multi-block allocations.
        let a = alloc.alloc_bulk([24u64, 60, 24]).unwrap();
        alloc.dealloc_bulk(a).unwrap();
        // Nothing leaked: the overhead of every freed block was cleared, so a
        // full recover scan finds the arena fully accounted for.
        assert_eq!(alloc.recover().unwrap(), 0, "no blocks should be leaked");

        // Reopen (runs recover automatically) and confirm the arena is usable.
        drop(alloc);
        let alloc2 = CheckedSlabBStackAllocator::open(BStack::open(&path).unwrap()).unwrap();
        let mut s = alloc2.alloc(24).unwrap();
        s.write([9u8; 24]).unwrap();
        assert_eq!(s.read().unwrap(), vec![9u8; 24]);
    }
    ...
    fn dealloc_bulk_rejects_double_free_and_returns_all_handles() {
        ...
        let _g = Guard(path);
        let alloc = CheckedSlabBStackAllocator::new(stack, 24).unwrap();
        // Free one block singly, then try to bulk-free it again alongside a live one.
        ...
        assert_eq!(err.source.kind(), std::io::ErrorKind::InvalidInput);
        // Rejected whole: both handles returned, nothing freed.
        assert_eq!(err.handles.len(), 2);
    }

    #[test]
    fn dealloc_bulk_rejects_repeated_handle_in_batch() {
        let (stack, path) = empty_stack();
        let _g = Guard(path);
        let alloc = CheckedSlabBStackAllocator::new(stack, 24).unwrap();
        let a = alloc.alloc(24).unwrap();
        let start = a.start();
        let dup = unsafe { crate::alloc::BStackOwnedSlice::from_raw_parts(&alloc, start, 24) };
        let err = alloc
            .dealloc_bulk([a, dup])
            .expect_err("a block appearing twice in one batch is a double free");
        assert_eq!(err.source.kind(), std::io::ErrorKind::InvalidInput);
        assert_eq!(err.handles.len(), 2);
    }
    ...
    fn dealloc_bulk_rejects_a_batch_with_a_foreign_handle() {
        ...
        let (s2, p2) = empty_stack();
        let _g2 = Guard(p2);
        let a1 = CheckedSlabBStackAllocator::new(s1, 32).unwrap();
        let a2 = CheckedSlabBStackAllocator::new(s2, 32).unwrap();
    ...
    fn bulk_round_trips_survive_reopen() {
        ...
        let _g = Guard(path.clone());
        let alloc = CheckedSlabBStackAllocator::new(stack, 24).unwrap();
        let mut slices = alloc.alloc_bulk([10u64, 60, 24]).unwrap();

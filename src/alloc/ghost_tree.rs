use super::{
    BStackAllocStats, BStackAllocator, BStackBulkAllocator, BStackSlice, ensure_own_slice,
    ensure_own_slices,
};
use crate::BStack;
...
impl GhostTreeBstackAllocator {
    ...
    /// allocation carries no header, leaving adjacent allocations to read as
    /// one run on disk; `in_use_blocks` therefore counts maximal contiguous
    /// live spans, the gaps between free nodes. `in_use_bytes` is the whole
    /// arena minus `free_bytes`.
    ///
    ...
    /// and the walk runs unlocked.
    ///
    /// # Errors
    ///
    /// [`io::ErrorKind::InvalidData`] where a cycle in a corrupt tree drives
    /// the traversal past the maximum AVL depth. Any other [`io::Error`] from
    /// the underlying [`BStack`] reads.
    pub fn stats(&self) -> io::Result<BStackAllocStats> {
        ...
        // `cursor` only ever moves forward, so an overlapping corrupt node
        // leaves each gap counted once.
...
mod tests {
    ...
    fn stats_counts_free_nodes_and_in_use_spans() {
        ...
        // a has b and c after it, so it becomes a second, separate free node.
        // GhostTree merges free neighbours only on the next open's
        // `coalesce_and_rebalance`. Only c is left live.
        alloc.dealloc(a).unwrap();
        assert_eq!(
            alloc.stats().unwrap(),
            BStackAllocStats {

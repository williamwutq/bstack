...
If an `alloc_bulk` step fails after popping, cleanup first truncates the fresh tail and returns the popped blocks to the free list; if the tail cannot be truncated, its blocks are recycled onto the free list too, so nothing leaks.
...
If an `alloc_bulk` step fails after popping, cleanup first truncates the fresh tail and returns the popped blocks to the free list; if the tail cannot be truncated, its blocks are recycled onto the free list too. Anything the best-effort cleanup misses is a zero-overhead leak `recover` reclaims.
